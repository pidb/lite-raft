...
pub mod mpsc {
    ...
    impl<T> Clone for WrapSender<T> {
        fn clone(&self) -> Self {
            Self {
                bounded_tx: self.bounded_tx.clone(),
                unbounded_tx: self.unbounded_tx.clone(),
            }
        }
    }
    ...
    unsafe impl<T> Send for WrapSender<T> {}
    unsafe impl<T> Sync for WrapSender<T> {}
    ...
    unsafe impl<T> Send for WrapReceiver<T> {}
    unsafe impl<T> Sync for WrapReceiver<T> {}
